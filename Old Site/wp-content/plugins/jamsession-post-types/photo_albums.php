...
add_action( 'init', 'JAMSESSION_SWP_create_photo_albums_post', 11);
function JAMSESSION_SWP_create_photo_albums_post() 
{
	...
	register_post_type( 'js_photo_albums',
		array(
			'labels' => array(
				'name' =>  __('Photo Albums', 'jamsession-post-types') ,
				'singular_name' =>  __('Photo Album', 'jamsession-post-types') ,
				'add_new' => __('Add New Photo Album', 'jamsession-post-types'),
				'add_new_item' => __('Add New Photo Album', 'jamsession-post-types'),
				'edit' => __('Edit', 'jamsession-post-types'),
				'edit_item' => __('Edit Photo Album', 'jamsession-post-types'),
				'new_item' => __('New Photo Album', 'jamsession-post-types'),
				'view' => __('View', 'jamsession-post-types'),
				'view_item' => __('View Photo Album', 'jamsession-post-types'),
				'search_items' => __('Search Photo Albums', 'jamsession-post-types'),
				'not_found' => __('No Photo Albums found','jamsession-post-types'),
				'not_found_in_trash' => __('No Photo Albums Found in Trash','jamsession-post-types'),
				'parent' => __('Parent Photo Album','jamsession-post-types')
			),
		'public' => true,
		'rewrite' => array(
			'slug' => $slug,
...
function JAMSESSION_SWP_photo_albums_admin_init() 
{
    add_meta_box( 'photo_albums_meta_box', 	/*the required HTML id attribute*/
        __('Image Gallery','jamsession-post-types'), 	/*text visible in the heading of meta box section*/
...
function JAMSESSION_SWP_display_photo_albums_meta_box($photoAlbumObject)
{
	...
	*/
	if (('' == trim($js_swp_gallery_images)) && ('' == $js_swp_gallery_compatibility242)) {
		...
		//$js_swp_gallery_images = JAMSESSION_SWP_get_old_style_images_attached($photoAlbumObject->ID);
		$js_swp_gallery_images_id = JAMSESSION_SWP_get_old_style_images_attached($photoAlbumObject->ID);
	}
	...
	*/
	if (('' != trim($js_swp_gallery_images)) && ('' != $js_swp_gallery_compatibility242) && ('' == $js_swp_gallery_compatibility243)) {
		$js_swp_gallery_images_id = JAMSESSION_SWP_get_ids_from_js_swp_gallery_images($js_swp_gallery_images);
	}
	...
	<div id="js_swp_cpt_gallery_container">
		<div id="js_swp_gallery_content">
			<?php JAMSESSION_SWP_fill_preview_container($js_swp_gallery_images_id); ?>
		</div>
		
		<div id="js_swp_add_image_container">
			<input type="button" id="js_swp_add_image_gallery_cpt" class="button button-primary" value="<?php echo __('Add Images To Gallery', 'jamsession-post-types'); ?>" />
		</div>
		
		<input type="text" name="js_swp_gallery_images" id="js_swp_gallery_images" value="<?php echo $js_swp_gallery_images; ?>" />
		<input type="text" name="js_swp_gallery_images_id" id="js_swp_gallery_images_id" value="<?php echo $js_swp_gallery_images_id; ?>" />
		
		<input type="text" name="js_swp_gallery_compatibility242" id="js_swp_gallery_compatibility242" value="<?php echo $js_swp_gallery_compatibility242; ?>" />
		<input type="text" name="js_swp_gallery_compatibility243" id="js_swp_gallery_compatibility243" value="<?php echo $js_swp_gallery_compatibility243; ?>" />
	</div>
...
function JAMSESSION_SWP_fill_preview_container($image_ids) {
	...
	foreach($idsArray as $imgID) {
		$imgTag = wp_get_attachment_image($imgID, 'medium');
		?>
		<li class="image_cell ui-sortable-handle" id="<?php echo $imgID; ?>">
			<?php echo $imgTag; ?>
			<div class="image_action remove_gallery_cpt_image" data-imid="<?php echo $imgID; ?>">Remove</div>
		</li>
		<?php
	}
	echo '</ul>';
...
*/
function JAMSESSION_SWP_get_old_style_images_attached($photo_album_ID) {
	$args = array(
	...
	foreach($loop as $image) {
		...
		$commaSeparatedImages .= '['.$thumbnail[0].']';
		*/
	}
...
function JAMSESSION_SWP_get_ids_from_js_swp_gallery_images($images_URLs) {
	$commaSeparatedIds = "";
	...
	foreach($imgArray as $imageURL) {
		...
		$imgId = JAMSESSION_SWP_get_attachment_id_from_url_jpt($imageURL);
...
add_action('save_post', 'JAMSESSION_SWP_save_photo_album_object', 10, 2);
function JAMSESSION_SWP_save_photo_album_object($gallery_id, $galleryObject) 
{
...
function JAMSESSION_SWP_display_gallery($photo_album_ID)
{
	...
	$gallery = '<ul id="js_gallery_admin">';
	$ii = 0;
	foreach( $loop as $image ):
		/*get the thumbnail*/
		...
		/*put it in html*/
		$gallery .= '<li class="image_cell" id="'.$image->ID.'">';
...
?>	
	<div id="js_swp_add_image_container">
		<input type="button" id="js_swp_add_image_gallery_cpt" class="button" value="<?php echo __('Add Images To Gallery', 'jamsession-post-types'); ?>" />
	</div>
...
function JAMSESSION_SWP_js_photo_albums_admin_columns_func( $columns)
{
	$columns = array(
		...
		'title' => __('Photo Album Title', 'jamsession-post-types'),
...
*/
add_action( 'init', 'JAMSESSION_SWP_create_photo_album_category', 11);

function JAMSESSION_SWP_create_photo_album_category()
{
	$slug = JAMSESSION_SWP_JPT_get_plugin_option("photo_album_tax");
	if ("" == $slug) {
	...
	register_taxonomy(
			...
			array(
				'labels' => array(
					'name' => __('Photo Album Categories', 'jamsession-post-types'),
					'singular_name'     => __( 'Photo Album Category', 'jamsession-post-types' ),
					'search_items'      => __( 'Search Photo Album Categories', 'jamsession-post-types'  ),
					'all_items'         => __( 'All Photo Album Categories', 'jamsession-post-types'  ),
					'parent_item'       => __( 'Parent Photo Album Category', 'jamsession-post-types'  ),
					'parent_item_colon' => __( 'Parent Photo Album Category:', 'jamsession-post-types'  ),
					'edit_item'         => __( 'Edit Photo Album Category' , 'jamsession-post-types' ),
					'update_item'       => __( 'Update Photo Album Category', 'jamsession-post-types'  ),
					'add_new_item' 		=> __('Add New Photo Album Category', 'jamsession-post-types'),
					'new_item_name' 	=> __('New Photo Album Category', 'jamsession-post-types'),
				),
				'rewrite' => array(
					'slug' => $slug,
					'with_front' => false
...
function JAMSESSION_SWP_get_attachment_id_from_url_jpt($attachment_url = '') {
...
add_action( 'wp_ajax_JAMSESSION_SWP_call_update_gallery_preview', 'JAMSESSION_SWP_call_update_gallery_preview');
function JAMSESSION_SWP_call_update_gallery_preview() {
	$ret['success'] = true;
	
	$imgIds = $_POST['image_attachment_ids'];

	ob_start();	
	echo '<div id="js_swp_gallery_content">';
	JAMSESSION_SWP_fill_preview_container($imgIds);
	echo '</div>';
