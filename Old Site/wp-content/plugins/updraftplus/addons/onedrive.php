...
class UpdraftPlus_Addons_RemoteStorage_onedrive extends UpdraftPlus_RemoteStorage_Addons_Base_v2 {
	...
	public function do_upload($file, $from) {
		...
		try {
			...
			// There's no obvious documentation, but this error code appears to be a token expiry: https://github.com/microsoftgraph/python-sample-auth/issues/10 . It would be nicer to request a new token immediately, but a swift resumption is not bad.
			if (false !== stripos($file_check_msg, 'token') && false !== strpos($file_check_msg, '80049228')) {
		...
		try {
			if (false != ($handle = fopen($from, 'rb'))) {
				if ($filesize < $this->chunk_size) {
					...
					if (true === $ret) $updraftplus->log_remove_warning('onedrive_expect_to_fail');
					return $ret;
				}
...
// $name = $this->onedrive_folder ? $this->onedrive_folder.'/'.$file : $file;
	...
	public function do_download($file, $fullpath, $start_offset) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
// return $obj->fetchContent();
	 ...
	 */
	public function do_authenticate_storage($instance_id) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
// $errs = $e->getErrors();
	...
	public function get_supported_features() {
		...
		return array('multi_options', 'config_templates', 'multi_storage', 'multi_delete');
	}
